...
async function init() {
  const tonweb = new TonWeb(
    new TonWeb.HttpProvider(
      is_prod ? process.env.main_rpc_network : process.env.test_rpc_network,
      {
        apiKey: is_prod ? process.env.main_apikey : process.env.testnet_apikey,
      }
    )
  );
  ...
  const getNftCollectionInfo = async () => {
    ...
    console.log(
      (await nftCollection.getNftItemAddressByIndex(new BN("1"))).toString(
        true,
        true,
        true
      )
  ...
  //const nftItemAddress = new TonWeb.utils.Address('xxx');
  ...
  const getSingleNftItemInfo = async () => {
    ...
    nftRoyalty.royaltyAddress = nftRoyalty.royaltyAddress.toString(
      true,
      true,
      true
    );
  ...
  const getSaleInfo = async () => {
    ...
    data.marketplaceAddress = data.marketplaceAddress.toString(
      true,
      true,
      true
    );
